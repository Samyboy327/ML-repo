...
def train_model(X_train, y_train, C=1.0, kernel="rbf", gamma="scale"):
    """Train SVC (SVM) wrapped in a StandardScaler pipeline."""
    pipeline = Pipeline([
...
def main():
    ...
    with mlflow.start_run(run_name=run_name) as run:
        # Log parameters (SVM hyperparameters now)
        C = 1.0
        kernel = "rbf"
        gamma = "scale"
        ...
        # Train model
        model = train_model(X_train, y_train, C=C, kernel=kernel, gamma=gamma)
